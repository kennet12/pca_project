$(document).on('click','.close-modal',function (e){
	$('.wrap-modal').removeClass('on');
	$('.wrap-modal').addClass('off');
});
...
{ 
	var str = '<div class="row">';
			...
			str += '<div class="col-md-6">';
				str += '<div class="box-modal">';
					...
					str += '<div class="modal-bottom">';
						str += '<button class="close-modal">Đóng</button>';
					str += '</div>';
				str += '</div>';
			str += '</div>';
			str += '<div class="col-md-3"></div>';
	...
	$(".modal-header").removeClass("error");
	$(".modal-header").removeClass("info");
	$(".modal-header").removeClass("success");
	
	if (type == "INFO") {
		$(".modal-header").addClass("info");
	...
	$(".wrap-modal").find(".modal-title").html(title);
	$(".wrap-modal").find(".modal-body").html(message);
	$(".wrap-modal").removeClass('off')
	$(".wrap-modal").addClass('on');
}

function showErrorMessage(arrMsg)
{
	var errmsg = "<p>Xảy ra những lỗi sau:</p>";
	errmsg += "<ul class='list-error'>";
