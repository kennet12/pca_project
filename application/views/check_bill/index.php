		...
		<div class="status">
			...
			<div class="orderstatus" data-id="<?=BOOKING_PREFIX.$booking->id?>">
				<label class="complete" status="0">
					<?  if ($booking->status == 1) {
							echo '<i class="fas fas-process fa-redo-alt"></i> Chờ xử lý ';
						} else if ($booking->status == 2) {
							echo '<i class="fas fas-process fa-clipboard-check"></i> Đã xác nhận ';
						} else if ($booking->status == 3) {
							echo '<i class="fas fas-process fa-shipping-fast"></i> Đang giao hàng ';
						} else if ($booking->status == 4) {
							echo '<i class="fas fas-process fa-check"></i> Giao thành công ';
						} 
					?>
