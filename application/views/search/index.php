		...
		<div class="col-md-3">
				...
				<div class="wrap-fill-box">
					...
					<div class="wrap-box-list" <?=$style?>>
						<div class="fill-box">
							<a href="<?=site_url('san-pham')?>"><h6 class="title">Tất cả sản phẩm</h6></a>
						</div>
						<div class="fill-box">
			...
			<div class="col-md-9">
				<div class="nav-list-product clearfix" id="list-product">
					<div class="float-left">
						<h6 class="render-result">Có <span class="text-color-red"><?=$total_items?></span> sản phẩm</h6>
					</div>
							...
							<div class="product">
								...
								<a title="<?=$item->title?>" href="<?=site_url($item->alias)?>">
									<div class="border-img">
										<img class="full-width hover-scale" src="<?=BASE_URL."/files/upload/product/{$item->code}/thumbnail/".end($photo);?>" alt="<?=$item->title?>" title="<?=$item->title?>">
									</div>
									<?
									$typename = !empty($item->typename)?' - '.$item->typename:'';
									$title_product = character_limiter($item->title.$typename,45);
									?>
									<h5 class="title"><?=$title_product?></h5>
									<div class="rating">
									...
									<div class="price"><?=number_format($item->price*(1-($item->sale*0.01)),0,',','.');?> ₫ 
									<? if(!empty($item->sale)) { ?>
									<span class="sale"><?=number_format($item->price,0,',','.');?> ₫</span>
									<? } ?>
									</div>
								</a>
							</div>
