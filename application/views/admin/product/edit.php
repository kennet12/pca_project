		...
		<table class="table table-bordered"><tr>
			...
			<td class="table-head text-right" width="10%">Mã</td>
				...
				<td>
					...
					<input type="hidden" id="category_main" name="category_main" class="form-control" value='<?=$item->category_id?>'>
				...
				<td>
					...
					<input type="hidden" id="category_main" name="category_main" class="form-control" value="">
					<script>
						...
						<div class="row">
							<? for ($i=0; $i<6; $i++) {
								...
								if (!empty($photo_list[$i])) {
									$photo = explode('/',$photo_list[$i]);
									$photo_name = end($photo);
									$bg_img = BASE_URL.'/files/upload/product/'.$item->code.'/thumbnail/'.$photo_name;
								}
								?>
							<div class="col-xs-6 col-sm-2">
								<div class="fileupload">
									<a class="btn-file-upload" file-id="<?=$i?>">
										<div class="bg <?=(!empty($photo_list[$i]) ? "selected" : "")?>" id="bg-<?=$i?>" style="<?=(!empty($photo_list[$i]) ? "background-image: url('{$bg_img}');" : "")?>"></div>
									</a>
									...
									<input type="file" class="userfile" id_item="<?=!empty($item) ? $item->id : 0?>" id="userfile-<?=$i?>" old_photo="<?=$photo_name?>" name="userfile_<?=$i?>" file-id="<?=$i?>" style="visibility: hidden;" />
								</div>
							</div>
							<? } ?>
						</div>
			...
			<tr>
				<td class="table-head text-right" width="10%">Thông tin SP</td>
...
<script>
	...
	$(document).on('change','.userfile',function() {
		...
		var file_path = BASE_URL+'/files/upload/product/'+code+'/'+new_photo+'.'+format;
