...
class Syslog extends CI_Controller {
			...
			if ($task == "save") {
				...
				if ($action == "add") {
					for ($i=0;$i<$c;$i++) {
						...
						$booking_id = $this->m_booking->get_auto_incre();
						$data_detail = array(
		...
		if (!empty($task)) {
			if ($task == "save") {
				...
				if (!empty($_FILES['thumbnail']['name'])){
					$data['thumbnail'] = "/images/slide/{$id}/{$this->util->slug($thumbnail[0])}.".end($thumbnail);
				}
				...
				$this->util->upload_file($path,'thumbnail',$file_deleted,$allow_type,$this->util->slug($thumbnail[0]).'.'.end($thumbnail),1050);
	...
	public function product($category_id=null, $action=null, $id=null)
	{
		...
		$info_type->product_id = !empty($id) ? $id : $this->m_product->get_auto_incre();
		$product_types = $this->m_product_type->items($info_type);
		if (!empty($task)) {
			if ($task == "save") {
				if (empty($id)) {
					$id = $this->m_product->get_auto_incre();
				} else {
					$info = new stdClass();
					$info->product_id = $id;
				...
				$typename			= $this->util->value($this->input->post("typename"), "");
				$product_company	= $this->util->value($this->input->post("product_company"), 1);
				$active				= $this->util->value($this->input->post("active"), 1);
	...
	public function enter_box($category,$action=null, $id=null)
	{
		...
		$info_detail = new stdClass();
		$info_detail->box_id = !empty($id) ? $id : $this->m_box->get_auto_incre();
		...
		if (!empty($task)) {
			if ($task == "save") {
				...
				if (empty($id)) {
					$id = $this->m_box->get_auto_incre();
				}
				$check_product_new = 0;
				if (empty($product_id)) {
					$product_id = $this->m_product->get_auto_incre();
					$check_product_new++;
				} else {
	...
	public function regulation ($action=null, $id=null) {
		$this->_breadcrumb = array_merge($this->_breadcrumb, array("Quy chế" => site_url("{$this->util->slug($this->router->fetch_class())}/{$this->router->fetch_method()}")));
		...
		if (!empty($task)) {
			if ($task == "save") {
				...
				// if (!empty($_FILES['thumbnail']['name'])){
				// 	$data['thumbnail'] = "/images/thumb/{$id}/{$this->util->slug($thumbnail[0])}.{$thumbnail[1]}";
				// }
				// $file_deleted = '';
				...
				$this->session->set_flashdata("success", "Update successfull");
				redirect(site_url("syslog/regulation"));
			}
		} else {
			$view_data = array();
			$view_data["breadcrumb"]	= $this->_breadcrumb;
	...
	public function blog_category ($category_id,$action=null, $id=null){
		$config_row_page = ADMIN_ROW_PER_PAGE;
		$pagi		= (isset($_GET["pagi"]) ? $_GET["pagi"] : $config_row_page);
		if (!isset($_GET['page']) || (($_GET['page']) < 1) ) {
			$page = 1;
		...
		if (!empty($task)) {
			if ($task == "save") {
				...
				redirect(site_url("syslog/blog-category/{$category_id}"));
			}
			else if ($task == "cancel") {
				redirect(site_url("syslog/blog-category/{$category_id}"));
			}
	...
	public function blog ($category_id, $action=null, $id=null) {
		...
		$this->_breadcrumb = array_merge($this->_breadcrumb, array("Blog/ Danh mục" => site_url("{$this->util->slug($this->router->fetch_class())}/blog-category/{$category->parent_id}")));
		$this->_breadcrumb = array_merge($this->_breadcrumb, array("{$category->name}" => site_url("{$this->util->slug($this->router->fetch_class())}/{$this->util->slug($this->router->fetch_method())}/{$category_id}")));
		...
		if (!empty($task)) {
			if ($task == "save") {
				...
				$this->create_sitemap();
				redirect(site_url("syslog/blog/{$category->id}"));
			}
			else if ($task == "cancel") {
				redirect(site_url("syslog/blog/{$category->id}"));
			}
		...
		if ($action == "add") {
			...
			$items = $this->m_contents->items($info, null, $pagi, $offset);
			if (!isset($_GET['pagi'])){
				$pagination = $this->util->pagination_admin(site_url('syslog/blog/'.$category->id). "?pagi=$config_row_page"."$_SERVER[QUERY_STRING]", $total, $pagi);
			}else{
				$pagination = $this->util->pagination_admin(site_url('syslog/blog/'.$category->id). "?$_SERVER[QUERY_STRING]", $total, $pagi);
			}
	...
	function upload_image_product($code)
	{
		...
		if ($this->upload->do_upload($file)) {
			...
			chmod("{$path_thumb}{$old_file_name}", 0777);
			unlink("{$path_thumb}{$old_file_name}");
		...
		$res = BASE_URL."/files/upload/product/{$code}/".$new_file_name.'.'.$format;
