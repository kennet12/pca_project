...
class Chi_tiet_san_pham extends CI_Controller {
	...
	public function index($alias) {
		...
		$this->_breadcrumb = array_merge($this->_breadcrumb, array("{$this->website['product']}" => site_url($this->alias['product'])));	
		...
		if (!empty($item)) {
			...
			foreach ($list_cateid as $cateid) {
				...
				$this->_breadcrumb = array_merge($this->_breadcrumb, array("{$category->{$this->prop['name']}}" => site_url("{$this->alias['product']}/{$category->{$this->prop['alias']}}")));
			}
