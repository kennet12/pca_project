<?php
class M_service extends M_db
{
	public function __construct()
	{
		parent::__construct();
		
		$this->_table = "m_service";
	}
	
	public function items($info=null, $active=null, $limit=null, $offset=null, $order_by=null, $sort_by='DESC')
	{
		$sql = "SELECT I.*, C.alias AS 'category_alias', '0' AS 'child_num' FROM {$this->_table} AS I INNER JOIN m_service_category AS C ON (I.category_id = C.id) WHERE 1 = 1";
		if (!is_null($info)) {
			if (!empty($info->category_id)) {
				$sql .= " AND I.category_id = '{$info->category_id}'";
			}
			if (!empty($info->type)) {
				$sql .= " AND I.type = '{$info->type}'";
			}
